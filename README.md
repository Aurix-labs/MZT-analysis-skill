...
MZT 采用**六阶段流程**（阶段 0–5），以"问题类型判断"作为前置阶段 0，后接四能力架构的四个主体阶段，并引入 **Gate 机制**强制 Agent 在关键节点等待用户确认：

| 阶段 | 说明 | Gate |
|------|------|------|
| **阶段 0** | 问题类型判断（前置） | Gate 0: 类型判断 Gate |
| **阶段 1** | 问题建构 | Gate 1: 问题确认 Gate |
| **阶段 2** | 方法论选择 | Gate 2: 方向确认 Gate |
| **阶段 3** | 分析输出 | Gate 3: 报告完成 Gate（等待用户评价） |
| **阶段 4** | 验证迭代 | - |
| **阶段 5** | 用户评价与反馈 | - |
...
Agent: 让我复述您的问题核心：您在考虑是否跳槽到创业公司，主要矛盾是职业发展机会与稳定性的权衡。这样理解准确吗？
